...
const ServiceCard = ({ icon, title, details, delay }) => {
  return (
    <div className="w-full px-4 md:w-1/2 lg:w-1/3">
      <motion.div 
        className="group mb-9 rounded-2xl bg-white p-8 shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1 h-[400px] flex flex-col"
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.5, delay }}
      >
        <div className="mb-6 flex h-[70px] w-[70px] items-center justify-center rounded-2xl bg-blue-600 transition-colors group-hover:bg-blue-700">
          {icon}
        </div>
        <h4 className="mb-4 text-2xl font-semibold text-gray-900">
          {title}
        </h4>
        <p className="text-lg leading-relaxed text-gray-600 flex-grow">
          {details}
        </p>
      </motion.div>
    </div>
